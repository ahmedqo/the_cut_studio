...
@section('content')
    <div class="flex flex-col gap-4">
        <div class="w-full flex items-center justify-between gap-2">
            <h1 class="font-bold text-2xl">{{ __('Create Type') }}</h1>
            <div
        ...
        <form id="form" action="{{ route('actions.types.store') }}" method="POST"
            ...
            <div class="flex flex-col gap-px">
                ...
                <textarea id="description" name="description" placeholder="{{ __('Description') }}" rows="2"
                    class="x-bg-custom text-[#1d1d1d] border-[#d1d1d1] focus-within:outline-primary p-2 text-base border rounded-md focus-within:outline focus-within:outline-2 focus-within:-outline-offset-2">{{ old('description') }}</textarea>
            </div>
        </form>
    </div>
@endsection
